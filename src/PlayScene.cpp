...
void PlayScene::update()
{
	...
	CollisionManager::AABBCheck(m_pSpaceShip, m_pObstacle); //checks to see if collision
}
...
void PlayScene::GUI_Function() const
{
	...
	ImGui::Begin("Game 3001 - Lab 2", NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_MenuBar);
	...
	if (ImGui::Button("Reset"))
	{
		...
		m_pSpaceShip->setEnabled(false);
		m_pSpaceShip->getRigidBody()->velocity = glm::vec2(0.0f); //reset velocity
		m_pSpaceShip->setRotation(0.0f); //reset angle
		turn_rate = 5.0f;
